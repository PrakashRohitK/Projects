...
def open_win():
   ...
   def database():
      ...
      if (id !=0 and name1 != '' and email != '' and gender != '' and state != '' and prog != '' and age !=0 and phone !=0 and re.fullmatch(regex, email) and addr!=''):
         ...
         tkinter.messagebox.showinfo("Registration Form", "Please Fill All the Fields correctly!!")
   ...
   addr_entry_9.place(x=270, y=450, width=200,height=70)
   ...
   c1_v1 = tk.StringVar(new)
   c1_v1.set('')

   b1=tk.Button(new,text='Submit',bg='lightyellow', font=20,state='disabled',command=database)
   b1.grid(row=20, column=1, padx=180, pady=590)
   Button(new, text='View Data', width=20, bg='white', fg='black', command=Table_format).place(x=400, y=590)
   c1 = tk.Checkbutton(new,text='I agree to the Terms and Conditions', variable=c1_v1,onvalue='Yes', offvalue='', command=my_check, font=24)
   c1.place(x=200,y=540)
   def display_msg():
